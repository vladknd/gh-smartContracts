...
# ============================================================================
# Governance Canister Test
# ============================================================================
# Tests the governance workflow:
# 1. Generate big book JSON (500+ nodes)
# 2. Stage content → 3. Create proposal → 4. Vote → 5. Execute → 6. Verify
#
# Purpose: Stress-test the governance system with:
...
echo -e "${BLUE}"
echo "============================================================================"
echo "     Governance Canister Stress Test"
echo "============================================================================"
echo -e "${NC}"
...
# ============================================================================
log_header "PHASE 0: Environment Setup"
...
if [ ! -f "$BIG_BOOK_JSON" ]; then
    log_fail "Big book JSON not generated"
    exit 1
fi
...
log_pass "Generated big book with $TOTAL_NODES nodes ($FILE_SIZE)"
...
if [ -z "$CONTENT_HASH" ]; then
    log_fail "Failed to stage content - no hash returned"
    exit 1
fi
...
if [[ "$STAGED_INFO" == *"Pending"* ]]; then
    log_pass "Staged content is in Pending status"
else
    log_fail "Unexpected staging status"
...
PYEOF
    ...
    if [[ "$SET_RESULT" == *"Ok"* ]]; then
        log_pass "Added current user as board member"
    elif [[ "$SET_RESULT" == *"locked"* ]]; then
        log_info "Board shares are locked - will use existing board member identity if available"
    else
        log_fail "Could not add board member: $SET_RESULT"
    fi
else
    log_pass "Current user is already a board member"
fi
...
if [[ "$PROPOSAL_RESULT" == *"Ok"* ]]; then
    ...
    log_pass "Content proposal created"
    log_info "Proposal ID: $PROPOSAL_ID"
else
    log_fail "Failed to create proposal: $PROPOSAL_RESULT"
fi
...
if [[ "$LINK_RESULT" == *"Ok"* ]] || [[ "$LINK_RESULT" == *"()"* ]]; then
    log_pass "Proposal linked to staged content"
else
...
if [[ "$VOTE_RESULT" == *"Ok"* ]]; then
    log_pass "Vote cast successfully"
else
...
if [[ "$CURRENT_STATUS" != *"Approved"* ]]; then
    ...
    if [[ "$FORCE_APPROVE" == *"Ok"* ]]; then
        log_pass "Proposal force-approved for testing"
    else
        log_fail "Failed to force-approve: $FORCE_APPROVE"
    fi
else
...
if [[ "$EXEC_RESULT" == *"Ok"* ]]; then
    log_pass "Proposal executed successfully - content loading initiated"
else
    log_fail "Failed to execute: $EXEC_RESULT"
fi
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
    ...
    if [[ "$LOADING_STATUS" == *"Completed"* ]]; then
        log_pass "Content loading completed!"
        break
        ...
        log_fail "Loading failed or paused"
        echo "$LOADING_STATUS"
        break
    fi
...
if [[ "$ROOT_CHECK" == *"$ROOT_ID"* ]]; then
    log_pass "Root book node found in learning engine!"
    ...
    log_fail "Root node not found in learning engine"
    echo "Expected ID: $ROOT_ID"
fi

# Verify quiz index
log_substep "Checking quiz index..."
# get_content_stats returns (node_count, quiz_count)
CONTENT_STATS_FULL=$(dfx canister call learning_engine get_content_stats 2>&1)
QUIZ_COUNT_EXTRACTED=$(echo "$CONTENT_STATS_FULL" | sed 's/.*, \([0-9_]*\) : nat64.*/\1/' | tr -d '_')
if [[ -n "$QUIZ_COUNT_EXTRACTED" ]]; then
    log_info "Quiz count from stats: $QUIZ_COUNT_EXTRACTED"
else
    log_info "Could not extract quiz count from: $CONTENT_STATS_FULL"
fi

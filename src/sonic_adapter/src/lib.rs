...
// Placeholder for Sonic canister ID
thread_local! {
    static SONIC_CANISTER_ID: RefCell<Principal> = RefCell::new(Principal::anonymous());
    static GHC_LEDGER_ID: RefCell<Principal> = RefCell::new(Principal::anonymous());
    static USDC_LEDGER_ID: RefCell<Principal> = RefCell::new(Principal::anonymous());
...
fn init(args: InitArgs) {
    SONIC_CANISTER_ID.with(|id| *id.borrow_mut() = args.sonic_canister_id);
    GHC_LEDGER_ID.with(|id| *id.borrow_mut() = args.ghc_ledger_id);
    USDC_LEDGER_ID.with(|id| *id.borrow_mut() = args.usdc_ledger_id);
    OWNER.with(|id| *id.borrow_mut() = args.owner);
}
...
// Structs removed as we use tuple arguments for dynamic calls
...
async fn launch_ico(args: LaunchIcoArgs) -> Result<String, String> {
    ...
    let sonic_id = SONIC_CANISTER_ID.with(|id| *id.borrow());
    let ghc_ledger = GHC_LEDGER_ID.with(|id| *id.borrow());
    let usdc_ledger = USDC_LEDGER_ID.with(|id| *id.borrow());
...
fn verify_owner() -> Result<(), String> {
    ...
    let owner = OWNER.with(|id| *id.borrow());
    if caller != owner {
...
async fn approve_token(ledger: Principal, spender: Principal, amount: u64) -> Result<(), String> {
    ...
    let (result,): (Result<candid::Nat, icrc_ledger_types::icrc2::approve::ApproveError>,) = ic_cdk::call(
        ledger,
        "icrc2_approve",
        (approve_args,)
